...
grain2(len, pos, pitch, x) = loop ~ _ : _ , _
    with {
        loop(y) = buffer2(offset + line, x) , crossfade(N, buffer2(offset + line, x) @ N, buffer2(offset - N + line, x)) 
            with {
                N = hslider("xfade", 32, 4, 1024, 1); // number of crossfading samples at the junction corresponding to the lookahead delay
...
pos = no.noise*size;
len = hslider("len", 192, 0, 19200, 1); // it should be more or equal to the length of the crossfade
...
//         rec = button("rec");
//     };
vol1 = hslider("vol1", 1, 0, 1, .001);
vol2 = hslider("vol2", 0, 0, 1, .001);
vol3 = hslider("vol3", 0, 0, 1, .001);

process = grain2(len, pos, pitch, in) : *(vol1) , *(vol1);
