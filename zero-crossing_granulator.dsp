...
with {
    ...
    with {
        ...
        shift(reset) = div((1 - pitch), rate) *
            line_reset(ba.sAndH(reset, rate), reset) ^ p_mod * ma.SR;
        // ZC POSITION FUNCTION
        zc_index(recall, x, y) = index - 
            ba.if(dir * diff(y) >= 0, zc_up, zc_down) : wrap(0, size)
        with {
            zc_up = recall , 
...
asynch_amount = hslider("[6]Position self-modulation depth", 0, 0, 1, .001) ^ 2 * 
    size * 16;
